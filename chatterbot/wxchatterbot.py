...
class ChatBot(object):
    ...
    def __init__(self, name, appid, encodingAESKey, token, **kwargs):
        self.appid = appid
        ...
        input_adapter = kwargs.get("input_adapter",
            "chatterbot.adapters.input.WXChat"
        )

        output_adapter = kwargs.get("output_adapter",
            "chatterbot.adapters.output.WXChat"
        )
    ...
    def wx_authentication(self, sign, timestamp, nonce):
    ...
    def get_response(self, input_item, **kwargs):
        """
        ...
        confidence, response = self.logic.process(input_statement, **kwargs)
